...
class QAService:
    def __init__(self, qa_repository, event_repository):
        print("QAService initialized with repositories")
    ...
    def save_question(self, question):
        ...
        event = current_app.event_service.get_event_by_room_id(question["room_id"])
        if not event:
            raise ValueError("Event not found")
        question["id"] = str(uuid.uuid4())
        question_id = self.qa_repository.save_question(question)
        user_email = current_app.auth_service.get_user_by_id(question["organizer_id"]).email
    ...
    def save_answer(self, answer):
        ...
        event = current_app.event_service.get_event_by_room_id(answer["room_id"])
        print(f"QAService.save_answer: Fetched event for room_id {answer['room_id']}: {event}")
        question = self.qa_repository.get_question_by_id(answer["question_id"])
        print(f"QAService.save_answer: Fetched question for question_id {answer['question_id']}: {question}")
        if not event:
            raise ValueError("Event not found")
        if not question:
            raise ValueError("Invalid question_id")
        answer["id"] = str(uuid.uuid4())
        print(f"QAService.save_answer: Saving answer with data: {answer}")
        answer_id = self.qa_repository.save_answer(answer)
        user_email = current_app.auth_service.get_user_by_id(answer["participant_id"]).email
