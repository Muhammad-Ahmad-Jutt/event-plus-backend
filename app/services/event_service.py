...
class EventService:
    ...
    def create_event(self, title, description, event_start_datetime,event_end_datetime, organizer_id, organizer_name=None, organizing_for=None, no_of_participants_allowed=10):
        ...
        return event
    def update_event(self, event_id, title=None, description=None, event_start_datetime=None,event_end_datetime=None, no_of_participants_allowed=None):
        ...
        if title is not None:
            slug = generate_slug(title)
            if self.event_repository.get_by_slug(slug) and self.event_repository.get_by_slug(slug).id != event_id:
                raise ValueError("An event with the same title already exists. Please choose a different title.")
            event.slug = slug
